...
公开媒体 URL 的查询参数为 `profile`、`ticket`、`expires` 和 `signature`，其中 profile 与 HMAC 保护的访问上下文都纳入签名，客户端不得修改。旧的无上下文签名不再接受。内容读取支持 `GET`、`HEAD`、单段 `Range`、`ETag/If-None-Match`；无效 Range 返回 `416`。普通响应可按完整签名 URL 和 Range 共享缓存最多 5 分钟；锁定、step-up 或分享上下文使用 `Cache-Control: no-store`。
