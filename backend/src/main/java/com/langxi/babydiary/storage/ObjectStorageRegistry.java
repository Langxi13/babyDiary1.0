...
public class ObjectStorageRegistry {
    ...
    public ObjectStorageRegistry(List<ObjectStorage> storages,
        ...
        if (!providers.containsKey(this.writeProvider)) {
            throw new IllegalStateException("Write object storage provider is unavailable: " + this.writeProvider);
        }
    }
    ...
    public ObjectStorage require(String provider) {
        ...
        if (storage == null) throw new IllegalStateException("Object storage provider is unavailable: " + provider);
        return storage;
    }
