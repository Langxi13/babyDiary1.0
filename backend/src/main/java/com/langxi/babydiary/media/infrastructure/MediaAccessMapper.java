...
public interface MediaAccessMapper {
    String PROTECTED_ASSET = "EXISTS (SELECT 1 FROM diary_media lock_dm JOIN diary lock_d "
    ...
    String DIRECT_SQL = "SELECT COUNT(*)>0 AS can_access,COALESCE(MAX(" + PROTECTED_ASSET
    ...
    String DIARY_SQL = "SELECT COUNT(*)>0 AS can_access,COALESCE(MAX(d.locked=1 OR " + PROTECTED_ASSET
    ...
    String ALBUM_SQL = "SELECT COUNT(*)>0 AS can_access,COALESCE(MAX(" + PROTECTED_ASSET
    ...
    String ANNIVERSARY_SQL = "SELECT COUNT(*)>0 AS can_access,COALESCE(MAX(" + PROTECTED_ASSET
    ...
    String AVATAR_SQL = "SELECT COUNT(*)>0 AS can_access,COALESCE(MAX(" + PROTECTED_ASSET
            ...
            + "AND a.deleted_at IS NULL AND a.status='READY'";
    String SHARE_SQL = "SELECT COUNT(*)>0 AS can_access,COALESCE(MAX(d.locked=1 OR " + PROTECTED_ASSET
    ...
    String PROTECTED_SQL = "SELECT " + PROTECTED_ASSET
            + " FROM diary_space s JOIN media_asset a ON a.space_id=s.space_id "
            + "WHERE s.public_id=#{spaceId} AND a.public_id=#{assetId}";

    @Select(DIRECT_SQL)
    AccessRow direct(@Param("spaceId") byte[] spaceId, @Param("assetId") byte[] assetId,
                     @Param("accountId") long accountId);

    @Select(DIARY_SQL)
    AccessRow diary(@Param("spaceId") byte[] spaceId, @Param("assetId") byte[] assetId,
                    @Param("contextId") byte[] contextId, @Param("accountId") long accountId);

    @Select(ALBUM_SQL)
    AccessRow album(@Param("spaceId") byte[] spaceId, @Param("assetId") byte[] assetId,
                    @Param("contextId") byte[] contextId, @Param("accountId") long accountId);

    @Select(ANNIVERSARY_SQL)
    AccessRow anniversary(@Param("spaceId") byte[] spaceId, @Param("assetId") byte[] assetId,
                          @Param("contextId") byte[] contextId, @Param("accountId") long accountId);

    @Select(AVATAR_SQL)
    AccessRow avatar(@Param("spaceId") byte[] spaceId, @Param("assetId") byte[] assetId,
                     @Param("contextId") byte[] contextId);

    @Select(SHARE_SQL)
    AccessRow share(@Param("spaceId") byte[] spaceId, @Param("assetId") byte[] assetId,
                    @Param("contextId") byte[] contextId);
    ...
    @Select("""
            ...
            """)
    List<byte[]> protectedAssets(@Param("spaceId")byte[]spaceId,@Param("assetIds")List<byte[]>assetIds);
