...
public interface SpaceMapper {
    ...
    @Insert(
            "INSERT INTO diary_space(public_id,name,type,created_by,personal_owner_id,default_visibility,storage_quota_bytes) "
                    + "VALUES(#{publicId},#{name},'PERSONAL',#{createdBy},#{createdBy},'PRIVATE',#{quotaBytes})")
    @Options(useGeneratedKeys = true, keyProperty = "spaceId")
    void insertPersonal(SpaceInsert row);

    @Insert(
            "INSERT INTO space_member(space_id,account_id,role,status) VALUES(#{spaceId},#{accountId},'OWNER','ACTIVE')")
    ...
    @Update(
            "UPDATE diary_space SET name=#{name},default_visibility=#{defaultVisibility} "
