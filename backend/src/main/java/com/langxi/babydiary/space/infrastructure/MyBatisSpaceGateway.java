...
public class MyBatisSpaceGateway implements SpaceGateway {
    ...
    public List<SpaceSummary> findForAccount(long accountId) {
        return mapper.findForAccount(accountId).stream()
                .map(
                        row ->
                                new SpaceSummary(
    ...
    public long insert(
        ...
        SpaceMapper.SpaceInsert row =
                new SpaceMapper.SpaceInsert(
        ...
        if (row.getSpaceId() == null)
            throw new IllegalStateException("Space insert returned no ID");
    ...
    public long insertPersonal(UUID publicId, String name, long ownerId, long quotaBytes) {
        ...
        if (row.getSpaceId() == null)
            throw new IllegalStateException("Personal space insert returned no ID");
        return row.getSpaceId();
    }
