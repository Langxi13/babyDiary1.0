...
public class SpaceController {
    ...
    public SpaceSummary create(@AuthenticationPrincipal AccountPrincipal principal,
                               @Valid @RequestBody CreateSpaceRequest request) {
        return spaces.create(principal.accountId(), request.name(), request.defaultVisibility());
    }

    @PutMapping("/{spaceId}")
    public SpaceSummary update(@AuthenticationPrincipal AccountPrincipal principal, @PathVariable UUID spaceId,
