...
public class MyBatisNotificationRepository implements NotificationRepository {
    ...
    public List<Row> findPage(long accountId, int limit, long offset) {
        return mapper.findPage(accountId, limit, offset).stream()
                .map(
                        row ->
                                new Row(
                                        BinaryUuid.fromBytes(row.publicId()),
                                        row.spacePublicId() == null
                                                ? null
    ...
    public long count(long accountId) {
        return mapper.count(accountId);
    }

    @Override
    public long countUnread(long accountId) {
        return mapper.countUnread(accountId);
    }

    @Override
    public void markRead(long accountId, UUID publicId, LocalDateTime now) {
        mapper.markRead(accountId, BinaryUuid.toBytes(publicId), now);
    }
