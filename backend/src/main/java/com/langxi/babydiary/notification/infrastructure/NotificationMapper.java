...
public interface NotificationMapper {
    @Select("""
            ...
            """)
    List<Row> findPage(@Param("accountId") long accountId, @Param("limit") int limit, @Param("offset") long offset);
    ...
    @Update("UPDATE notification SET read_at=COALESCE(read_at,#{now}) WHERE account_id=#{accountId} AND public_id=#{publicId}")
    void markRead(@Param("accountId") long accountId, @Param("publicId") byte[] publicId,
                  @Param("now") LocalDateTime now);

    @Update("UPDATE notification SET read_at=#{now} WHERE account_id=#{accountId} AND read_at IS NULL")
