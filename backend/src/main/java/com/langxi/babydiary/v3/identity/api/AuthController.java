...
import com.langxi.babydiary.v3.identity.application.AuthenticationService;
import com.langxi.babydiary.v3.identity.application.StepUpService;
import com.langxi.babydiary.v3.identity.application.RegistrationService;
...
public class AuthController {
    ...
    public AuthController(AuthenticationService authentication, StepUpService stepUp, RegistrationService registrations,
    ...
    public ResponseEntity<SessionResponse> login(@Valid @RequestBody LoginRequest request,
        ...
        rateLimiter.require("login", rateLimiter.client(servletRequest) + ":" + request.username(), 10, 900);
        AuthenticationService.Session session = authentication.login(request.username(), request.password(),
                new AuthenticationService.Device(deviceName, servletRequest.getHeader(HttpHeaders.USER_AGENT),
    ...
    public void register(@Valid @RequestBody RegisterRequest request, HttpServletRequest servletRequest) {
        ...
        registrations.register(request.username(), request.password(), request.confirmPassword(), request.invitationCode());
    }
    ...
    @PostMapping("/step-up")
    public StepUpResponse stepUp(@AuthenticationPrincipal V3Principal principal, @Valid @RequestBody PasswordRequest request,
                                 HttpServletRequest servletRequest) {
        rateLimiter.require("step-up", rateLimiter.client(servletRequest) + ":" + principal.accountId(), 10, 900);
    ...
    public java.util.List<AuthenticationService.SessionView> sessions(
            @AuthenticationPrincipal V3Principal principal,
            @CookieValue(value = REFRESH_COOKIE, required = false) String refreshToken) {
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void revokeSession(@AuthenticationPrincipal V3Principal principal, @PathVariable UUID sessionId) {
    ...
    private ResponseEntity<SessionResponse> response(AuthenticationService.Session session) {
        SessionResponse body = new SessionResponse(session.accountId(), session.username(), session.email(),
                session.role(), session.timezone(), session.accessToken(), session.accessExpiresAt());
    ...
    public record SessionResponse(UUID accountId, String username, String email, String role,
                                  String timezone, String accessToken, long accessExpiresAt) {
