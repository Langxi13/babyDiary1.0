...
import com.langxi.babydiary.v3.space.application.CollaborationRepository;
...
public interface CollaborationMapper {
    ...
    @Select("""
            ...
            """)
    InvitationRow findInvitation(@Param("tokenHash") byte[] tokenHash, @Param("now") LocalDateTime now);
    ...
    @Update("""
            ...
            """)
    int acceptInvitation(@Param("invitationId") long invitationId, @Param("accountId") long accountId,
                         @Param("now") LocalDateTime now);

    @Insert("INSERT INTO space_member(space_id,account_id,role,status,joined_at) VALUES(#{spaceId},#{accountId},#{role},'ACTIVE',UTC_TIMESTAMP(6))")
    void insertMember(@Param("spaceId") long spaceId, @Param("accountId") long accountId,
                      @Param("role") String role);

    @Select("SELECT m.account_id,m.role,m.status FROM space_member m JOIN account a ON a.account_id=m.account_id " +
            "WHERE m.space_id=#{spaceId} AND a.public_id=#{publicId}")
    MembershipRow findMembership(@Param("spaceId") long spaceId, @Param("publicId") byte[] publicId);
    ...
    @Update("UPDATE space_member SET role=#{role},status='ACTIVE' WHERE space_id=#{spaceId} AND account_id=#{accountId}")
    int updateRole(@Param("spaceId") long spaceId, @Param("accountId") long accountId, @Param("role") String role);

    @Update("UPDATE space_member SET status='LEFT' WHERE space_id=#{spaceId} AND account_id=#{accountId} AND status='ACTIVE'")
    ...
    final class InvitationInsert {
        ...
        public InvitationInsert(byte[] publicId, long spaceId, long invitedBy, String email, byte[] tokenHash,
    ...
    record InvitationRow(long invitationId, long spaceId, String email, String role, LocalDateTime expiresAt) {
    }
