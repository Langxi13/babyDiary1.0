package com.langxi.babydiary.v3.tag.application;
...
public class TagService {
    ...
    public Tag create(UUID spaceId, long accountId, String name, String color) {
        ...
        if (normalizedName.isBlank()) throw V3Exception.badRequest("TAG_NAME_REQUIRED", "标签名称不能为空");
        if (normalizedName.length() > 32) throw V3Exception.badRequest("TAG_NAME_TOO_LONG", "标签名称不能超过32个字符");
        ...
        if (normalizedColor != null && !COLOR.matcher(normalizedColor).matches()) {
            throw V3Exception.badRequest("TAG_COLOR_INVALID", "标签颜色格式无效");
        }
        ...
        try {
            ...
            throw new V3Exception(org.springframework.http.HttpStatus.CONFLICT, "TAG_NAME_EXISTS", "当前空间已存在同名标签");
        }
    }
}
