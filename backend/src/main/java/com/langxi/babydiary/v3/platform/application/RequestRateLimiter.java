...
public class RequestRateLimiter {
    ...
    public RequestRateLimiter(ObjectProvider<StringRedisTemplate> redis,
                              @Value("${app.v3.rate-limit.enabled:true}") boolean enabled,
                              @Value("${app.v3.rate-limit.redis-enabled:false}") boolean redisEnabled) {
    ...
    public void require(String scope, String identity, int limit, long windowSeconds) {
        ...
        if (count > limit) {
            throw new V3Exception(HttpStatus.TOO_MANY_REQUESTS, "RATE_LIMITED", "请求过于频繁，请稍后重试");
        }
    }
    ...
    private long redisCount(String key, long windowSeconds) {
        ...
        try {
            Long value = redis.execute(INCREMENT, List.of(key), Long.toString(windowSeconds * 1000));
            return value == null ? -1 : value;
        } catch (RuntimeException exception) {
            log.warn("Redis rate limiter unavailable; using local fallback: {}", exception.getMessage());
            return -1;
        }
    }
    ...
    private long localCount(String key, long windowSeconds) {
        ...
        Window value = local.compute(key, (ignored, current) ->
    ...
    private void cleanup(long now) {
        ...
        if (local.size() > MAX_LOCAL_KEYS) {
            local.keySet().stream().limit(local.size() - MAX_LOCAL_KEYS).toList().forEach(local::remove);
        }
    }
