...
import com.langxi.babydiary.v3.media.infrastructure.MediaAccessMapper;
...
public class MediaAccessPolicy {
  private final MediaAccessMapper access;

  public MediaAccessPolicy(MediaAccessMapper access) {
    this.access = access;
  }
  ...
  public void require(UUID spaceId, UUID assetId, MediaAccessContext context) {
    ...
    byte[] parent = context.contextId() == null ? null : BinaryUuid.toBytes(context.contextId());
    MediaAccessMapper.AccessRow row =
        switch (context.source()) {
          ...
          case ANNIVERSARY -> access.anniversary(space, asset, parent, context.accountId());
          case AVATAR -> access.avatar(space, asset, parent);
          case SHARE -> access.share(space, asset, parent);
        };
    if (row == null || !row.canAccess()) {
      throw V3Exception.notFound("MEDIA_NOT_FOUND", "媒体不存在或无权访问");
    }
  ...
  public Set<UUID> protectedAssets(UUID spaceId, List<UUID> assetIds) {
    ...
    return access
        .protectedAssets(
            BinaryUuid.toBytes(spaceId), assetIds.stream().map(BinaryUuid::toBytes).toList())
        .stream()
