...
public class AuthenticationService {
    ...
    public Session refresh(String currentRefreshToken) {
        ...
        AccountGateway.RefreshSession session = accounts.findRefreshSession(previousHash, now)
                ...
                .orElseThrow(() -> new V3Exception(org.springframework.http.HttpStatus.UNAUTHORIZED,
        ...
        if (!accounts.rotateRefreshToken(session.sessionId(), previousHash, hash(nextRefreshToken), now)) {
            throw new V3Exception(org.springframework.http.HttpStatus.UNAUTHORIZED,
    ...
    public void revokeSession(long accountId, UUID sessionId) {
        if (!accounts.revokeSession(accountId, sessionId, LocalDateTime.now(clock))) {
            throw V3Exception.notFound("SESSION_NOT_FOUND", "登录设备不存在或已经退出");
        }
    }
    ...
    private byte[] hash(String value) {
        if (value == null || value.isBlank()) throw new V3Exception(org.springframework.http.HttpStatus.UNAUTHORIZED,
    ...
    private V3Exception invalidCredentials() {
        return new V3Exception(org.springframework.http.HttpStatus.UNAUTHORIZED,
