...
public class ReminderService {
    ...
    public ReminderService(SpaceAccess spaces, ReminderRepository reminders, ObjectMapper json,
    ...
    @Transactional
    public ReminderRepository.Row save(UUID spaceId, long accountId, String type, String time,
                                       Integer dayOfWeek, boolean enabled) {
        ...
        if (!"DAILY".equals(normalized) && !"WEEKLY".equals(normalized)) {
            throw V3Exception.badRequest("REMINDER_TYPE_INVALID", "提醒类型无效");
        }
        LocalTime parsed;
        try {
            parsed = LocalTime.parse(time);
        } catch (Exception exception) {
            throw V3Exception.badRequest("REMINDER_TIME_INVALID", "提醒时间无效");
        }
        if ("WEEKLY".equals(normalized) && (dayOfWeek == null || dayOfWeek < 1 || dayOfWeek > 7)) {
            throw V3Exception.badRequest("REMINDER_DAY_INVALID", "每周提醒需要选择1至7的星期");
        }
        ...
        ZoneId zone;
        try {
            zone = ZoneId.of(profiles.find(accountId).map(ProfileRepository.Profile::timezone).orElse("Asia/Shanghai"));
        } catch (Exception exception) {
            zone = ZoneId.of("Asia/Shanghai");
        }
        LocalDateTime next = enabled ? next(normalized, parsed, dayOfWeek, ZonedDateTime.now(zone)) : null;
        reminders.upsert(new ReminderRepository.NewReminder(BinaryUuid.toBytes(UUID.randomUUID()), accountId, space.internalId(), normalized,
                enabled, schedule.toString(), next));
        return reminders.findForAccount(space.internalId(), accountId).stream().filter(row -> normalized.equals(row.type())).findFirst()
    ...
    private LocalDateTime next(String type, LocalTime time, Integer day, ZonedDateTime now) {
        ...
        if ("DAILY".equals(type)) {
            ...
            LocalDate date = now.toLocalDate().with(TemporalAdjusters.nextOrSame(DayOfWeek.of(day)));
            candidate = date.atTime(time).atZone(now.getZone());
            if (!candidate.isAfter(now)) candidate = candidate.plusWeeks(1);
        }
        return candidate.withSecond(0).withNano(0).withZoneSameInstant(ZoneOffset.UTC).toLocalDateTime();
    }
}
