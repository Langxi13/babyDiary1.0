...
public class PrivateShareService {
  ...
  public Created create(
    ...
    if (diary.isLocked())
      throw V3Exception.conflict("LOCKED_CONTENT_NOT_SHAREABLE", "锁定日记不能创建公开分享");
    if (hours < 1 || hours > 720)
      throw V3Exception.badRequest("SHARE_EXPIRY_INVALID", "分享有效期应为1小时到30天");
    if (maxViews != null && (maxViews < 1 || maxViews > 10000))
      throw V3Exception.badRequest("SHARE_VIEWS_INVALID", "浏览次数应为1到10000");
    ...
    if (normalized != null && (normalized.length() < 4 || normalized.length() > 64))
      throw V3Exception.badRequest("SHARE_PASSWORD_INVALID", "分享密码长度应为4到64位");
  ...
  public void revoke(UUID shareId, long accountId) {
    if (mapper.revoke(BinaryUuid.toBytes(shareId), accountId) != 1)
      throw V3Exception.notFound("SHARE_NOT_FOUND", "分享不存在或无权撤销");
  }

  @Transactional
  public SharedDiary open(String token, String password) {
    if (token == null || token.isBlank())
      throw V3Exception.notFound("SHARE_NOT_FOUND", "分享不存在或已过期");
    ...
    if (row.getPasswordHash() != null
      ...
      throw new V3Exception(HttpStatus.UNAUTHORIZED, "SHARE_PASSWORD_INVALID", "分享密码不正确");
  ...
  private PrivateShareMapper.DiaryRow manageable(
    ...
    if (row == null) throw V3Exception.notFound("DIARY_NOT_FOUND", "日记不存在或无权管理分享");
    return row;
  }
