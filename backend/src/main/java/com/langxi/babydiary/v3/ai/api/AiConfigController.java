...
import com.langxi.babydiary.v3.ai.application.AiConfigService;
...
public class AiConfigController {
    ...
    @GetMapping
    public AiConfigService.ConfigView view(@AuthenticationPrincipal V3Principal principal) {
        return ai.view(principal);
    }

    @PostMapping
    public AiConfigService.ConfigView save(@AuthenticationPrincipal V3Principal principal,
    ...
    @PostMapping("/test")
    public TestResponse test(@AuthenticationPrincipal V3Principal principal) {
        return new TestResponse(ai.test(principal));
    }

    @GetMapping("/models")
    public List<String> models(@AuthenticationPrincipal V3Principal principal) {
