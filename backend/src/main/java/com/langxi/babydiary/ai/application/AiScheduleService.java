...
import com.langxi.babydiary.ai.infrastructure.AiScheduleMapper;
...
public class AiScheduleService {
    ...
    @Transactional public Schedule update(UUID spaceId,long accountId,boolean weekly,boolean monthly,boolean annual){
        SpaceAccess.SpaceContext space=spaces.requireMember(spaceId,accountId);if(!"OWNER".equals(space.role()))throw ApiException.forbidden("SPACE_OWNER_REQUIRED","只有空间所有者可以修改自动回顾计划");
