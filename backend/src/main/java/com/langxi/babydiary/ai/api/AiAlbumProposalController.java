...
import com.langxi.babydiary.ai.application.AiAlbumProposalService;
...
public class AiAlbumProposalController {
    ...
    public AiAlbumProposalController(
            AiAlbumProposalService proposals, MediaRepresentationService media) {
    ...
    public ProposalResponse detail(
            @AuthenticationPrincipal AccountPrincipal principal,
            @PathVariable UUID spaceId,
            @PathVariable UUID proposalId) {
        return response(
                principal.accountId(),
                proposals.detail(spaceId, principal.accountId(), proposalId));
    }
    ...
    public ProposalResponse confirm(
            @AuthenticationPrincipal AccountPrincipal principal,
            @PathVariable UUID spaceId,
            @PathVariable UUID proposalId) {
        return response(
                principal.accountId(),
                proposals.confirm(spaceId, principal.accountId(), proposalId));
    }
    ...
    private ProposalResponse response(long accountId, AiAlbumProposalService.Proposal proposal) {
        return new ProposalResponse(
                ...
                proposal.prompt(),
                proposal.model(),
                proposal.albums().stream()
                        .map(
                                candidate ->
                                        new CandidateResponse(
                                                candidate.mode(),
                                                candidate.targetAlbumId(),
                                                candidate.targetAlbumName(),
                                                candidate.title(),
    ...
    public record UpdateRequest(
            @NotNull @Size(min = 1, max = 50) List<@Valid CandidateRequest> albums) {}
    ...
    public record CandidateRequest(
            ...
            @Size(max = 500) List<UUID> diaryIds,
            @Size(max = 500) List<UUID> assetIds,
            boolean discarded) {
        AiAlbumProposalService.CandidateCommand command() {
            return new AiAlbumProposalService.CandidateCommand(
                    mode,
                    targetAlbumId,
                    title,
                    description,
                    diaryIds == null ? List.of() : diaryIds,
