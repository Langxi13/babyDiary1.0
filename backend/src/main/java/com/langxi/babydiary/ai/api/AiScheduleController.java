...
public class AiScheduleController {
    ...
    public AiScheduleController(AiScheduleService schedules) {
        this.schedules = schedules;
    }

    @GetMapping
    public AiScheduleService.Schedule get(
            @AuthenticationPrincipal AccountPrincipal p, @PathVariable UUID spaceId) {
        return schedules.get(spaceId, p.accountId());
    }

    @PutMapping
    public AiScheduleService.Schedule update(
            @AuthenticationPrincipal AccountPrincipal p,
            @PathVariable UUID spaceId,
            @RequestBody Request r) {
        return schedules.update(
                spaceId, p.accountId(), r.weeklyEnabled, r.monthlyEnabled, r.annualEnabled);
    }
