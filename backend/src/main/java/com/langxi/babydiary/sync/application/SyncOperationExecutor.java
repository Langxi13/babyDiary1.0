...
public class SyncOperationExecutor {
    ...
    public Result execute(
            UUID spaceId,
            long internalSpaceId,
            long accountId,
            boolean elevated,
            Operation operation) {
        SyncRepository.OperationResult previous =
                operations.findOperation(operation.operationId(), accountId, internalSpaceId);
        if (previous != null) {
            return new Result(
        ...
        try {
            ...
            String status =
                    exception.status() == HttpStatus.PRECONDITION_FAILED ? "CONFLICT" : "FAILED";
            result =
                    new Result(
                            operation.operationId(),
                            status,
        ...
        if (!operations.insertOperation(
            ...
            SyncRepository.OperationResult concurrent =
                    operations.findOperation(operation.operationId(), accountId, internalSpaceId);
            if (concurrent != null) {
                return new Result(
                        operation.operationId(),
                        concurrent.resultCode(),
                        concurrent.entityId(),
                        null,
                        null,
                        null);
                ...
                switch (operation.action()) {
                    case "CREATE" ->
                            diaries.create(
                                    spaceId,
                                    accountId,
                                    command(operation.entityId(), operation.payload()));
                    case "UPDATE" ->
                            diaries.update(
                                    spaceId,
                                    requiredEntity(operation),
                                    accountId,
                                    requiredVersion(operation),
                                    command(null, operation.payload()),
                                    elevated);
                    case "DELETE" -> {
                        diaries.moveToTrash(
                                spaceId,
                    ...
                    case "RESTORE" ->
                            diaries.restore(
                                    ...
                                    requiredVersion(operation),
                                    elevated);
                    default -> throw ApiException.badRequest("SYNC_ACTION_INVALID", "同步动作无效");
                };
    ...
    private DiaryService.Command command(UUID clientId, JsonNode payload) {
        if (payload == null || !payload.isObject()) {
            throw ApiException.badRequest("SYNC_PAYLOAD_REQUIRED", "同步操作缺少日记内容");
        }
        String date = text(payload, "diaryDate", text(payload, "date", null));
        if (date == null) throw ApiException.badRequest("DIARY_DATE_REQUIRED", "请选择日记日期");
        return new DiaryService.Command(
                ...
                payload.path("locked").asBoolean(false),
                uuids(payload.path("tagIds")),
                uuids(payload.path("mediaIds")));
    }

    private UUID requiredEntity(Operation operation) {
        if (operation.entityId() == null)
            throw ApiException.badRequest("SYNC_ENTITY_REQUIRED", "同步操作缺少日记标识");
        return operation.entityId();
    }

    private int requiredVersion(Operation operation) {
        if (operation.baseVersion() == null || operation.baseVersion() < 1) {
            throw ApiException.badRequest("SYNC_VERSION_REQUIRED", "同步操作缺少有效版本");
        }
