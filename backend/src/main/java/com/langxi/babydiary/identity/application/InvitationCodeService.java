...
import com.langxi.babydiary.platform.infrastructure.AesGcmSecretCodec;
...
public class InvitationCodeService implements ApplicationRunner {
    ...
    public String view(AccountPrincipal principal, String token) {
        ...
        if (encrypted == null || encrypted.isBlank()) throw ApiException.notFound("INVITATION_CODE_MISSING", "邀请码尚未配置");
        try {
    ...
    private void verify(AccountPrincipal principal, String token) {
        if (principal == null || !"ADMIN".equals(principal.role())) throw ApiException.forbidden("ADMIN_REQUIRED", "只有管理员可以管理邀请码");
