...
public interface AccountRecoveryMapper {
    @Select(
            "SELECT account_id,email,email_verified FROM account WHERE account_id=#{accountId} AND deleted_at IS NULL")
    AccountEmailRow findEmailByAccount(long accountId);

    @Select(
            "SELECT account_id,email,email_verified FROM account WHERE email=#{email} AND deleted_at IS NULL")
    AccountEmailRow findByEmail(String email);

    @Select(
            "SELECT account_id,password_hash FROM account WHERE username=#{username} AND deleted_at IS NULL")
    AccountPasswordRow findByUsername(String username);

    @Update(
            "UPDATE account SET email=#{email},email_verified=0,updated_at=UTC_TIMESTAMP(6) "
                    + "WHERE account_id=#{accountId} AND deleted_at IS NULL")
    int updateEmail(@Param("accountId") long accountId, @Param("email") String email);

    @Insert(
            "INSERT INTO account_token(account_id,type,token_hash,expires_at) "
    ...
    @Delete(
            "DELETE FROM account_token WHERE account_id=#{accountId} AND type=#{type} AND used_at IS NULL")
    void deleteTokens(@Param("accountId") long accountId, @Param("type") String type);

    @Select(
            "SELECT token_id,account_id FROM account_token WHERE token_hash=#{tokenHash} AND type=#{type} "
                    + "AND used_at IS NULL AND expires_at>#{now} FOR UPDATE")
    TokenRow findTokenForUpdate(
            @Param("tokenHash") byte[] tokenHash,
            @Param("type") String type,
            @Param("now") LocalDateTime now);

    @Update(
            "UPDATE account_token SET used_at=#{now} WHERE token_id=#{tokenId} AND used_at IS NULL AND expires_at>#{now}")
    int consumeToken(@Param("tokenId") long tokenId, @Param("now") LocalDateTime now);

    @Update(
            "UPDATE account SET email_verified=1,updated_at=UTC_TIMESTAMP(6) WHERE account_id=#{accountId}")
    ...
    @Insert(
            "<script>INSERT INTO recovery_code(account_id,code_hash) VALUES "
                    + "<foreach collection='hashes' item='hash' separator=','>(#{accountId},#{hash})</foreach></script>")
    void insertRecoveryCodes(
            @Param("accountId") long accountId, @Param("hashes") List<byte[]> hashes);

    @Update(
            "UPDATE recovery_code SET used_at=#{now} WHERE account_id=#{accountId} AND code_hash=#{codeHash} AND used_at IS NULL")
