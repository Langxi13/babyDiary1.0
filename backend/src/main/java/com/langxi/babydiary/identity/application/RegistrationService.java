...
public class RegistrationService {
    ...
    public void register(String username, String password, String confirmation, String invitationCode) {
        ...
        if (!password.equals(confirmation)) throw ApiException.badRequest("PASSWORD_MISMATCH", "两次输入的密码不一致");
        if (!invitations.matchesForRegistration(invitationCode)) throw ApiException.badRequest("INVITATION_CODE_INVALID", "邀请码无效");
        try {
            ...
            throw new ApiException(org.springframework.http.HttpStatus.CONFLICT, "USERNAME_EXISTS", "用户名已存在");
        }
    }
}
