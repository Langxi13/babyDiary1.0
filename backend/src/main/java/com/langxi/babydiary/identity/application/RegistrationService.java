...
public class RegistrationService {
    ...
    public void register(
            String username, String password, String confirmation, String invitationCode) {
        ...
        if (password == null || password.length() < 8 || password.length() > 200) {
            throw ApiException.badRequest("PASSWORD_WEAK", "密码长度需为8至200个字符");
        }
        if (!password.equals(confirmation))
            throw ApiException.badRequest("PASSWORD_MISMATCH", "两次输入的密码不一致");
        if (!invitations.matchesForRegistration(invitationCode))
            throw ApiException.badRequest("INVITATION_CODE_INVALID", "邀请码无效");
        try {
            boolean firstAccount = accounts.countAccounts() == 0;
            long accountId =
                    accounts.insertAccount(
                            UUID.randomUUID(), normalized, passwords.encode(password));
            if (firstAccount) accounts.promoteToAdmin(accountId);
            long spaceId =
                    spaces.insertPersonal(
                            UUID.randomUUID(), normalized + "的日记", accountId, DEFAULT_QUOTA);
            spaces.insertOwner(spaceId, accountId);
            spaces.insertStorageUsage(spaceId);
        } catch (DuplicateKeyException exception) {
            throw new ApiException(
                    org.springframework.http.HttpStatus.CONFLICT, "USERNAME_EXISTS", "用户名已存在");
        }
    }
}
