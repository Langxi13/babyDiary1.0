...
public interface IdentityMapper {
    ...
    @Insert(
            "INSERT INTO account(public_id,username,password_hash,system_role,timezone,status,created_at,updated_at) "
    ...
    @Update(
            "UPDATE account SET system_role='ADMIN',updated_at=UTC_TIMESTAMP(6) WHERE account_id=#{accountId}")
    void promoteToAdmin(long accountId);

    @Select(
            "SELECT public_id,device_name,user_agent,ip_address,expires_at,last_seen_at,created_at FROM auth_session "
    ...
    @Select(
            "SELECT BIN_TO_UUID(public_id) FROM auth_session WHERE account_id=#{accountId} AND refresh_token_hash=#{refreshHash} "
    ...
    @Update(
            "UPDATE auth_session SET revoked_at=#{now} WHERE account_id=#{accountId} AND public_id=#{publicId} AND revoked_at IS NULL")
    int revokeSession(
            @Param("accountId") long accountId,
    ...
    RefreshSessionRow findRefreshSession(
            @Param("refreshHash") byte[] refreshHash, @Param("now") LocalDateTime now);
