...
public interface DiaryInteractionMapper {
    @Select(
            ...
            """)
    DiaryAccessRow findVisibleDiary(
            @Param("spaceId") long spaceId,
            @Param("diaryPublicId") byte[] diaryPublicId,
    ...
    @Insert(
            "INSERT INTO diary_comment(public_id,diary_id,author_id,content) "
                    + "VALUES(#{publicId},#{diaryId},#{authorId},#{content})")
    @Options(useGeneratedKeys = true, keyProperty = "commentId")
    void insertComment(CommentInsert row);

    @Update(
            "UPDATE diary_comment SET content=#{content},updated_at=UTC_TIMESTAMP(6) "
    ...
    @Update(
            "UPDATE diary_comment SET deleted_at=UTC_TIMESTAMP(6),updated_at=UTC_TIMESTAMP(6) "
                    + "WHERE diary_id=#{diaryId} AND public_id=#{publicId} AND author_id=#{authorId} AND deleted_at IS NULL")
    int deleteComment(
            @Param("diaryId") long diaryId,
            @Param("publicId") byte[] publicId,
    ...
    @Select(
            ...
            """)
    List<ReactionRow> findReactions(
            @Param("diaryId") long diaryId, @Param("accountId") long accountId);

    @Insert(
            "INSERT IGNORE INTO diary_reaction(diary_id,account_id,emoji) VALUES(#{diaryId},#{accountId},#{emoji})")
    void insertReaction(
            @Param("diaryId") long diaryId,
            @Param("accountId") long accountId,
    ...
    @Delete(
            "DELETE FROM diary_reaction WHERE diary_id=#{diaryId} AND account_id=#{accountId} AND emoji=#{emoji}")
    void deleteReaction(
            @Param("diaryId") long diaryId,
            @Param("accountId") long accountId,
            @Param("emoji") String emoji);
    ...
    final class CommentRow {
        ...
        private byte[] authorPublicId;
        private String username;
        private String content;
        private LocalDateTime createdAt;
        private LocalDateTime updatedAt;
        private byte[] avatarAssetPublicId;
        ...
        public byte[] getAvatarAssetPublicId() {
            return avatarAssetPublicId;
        }

        public byte[] getAvatarSpacePublicId() {
            return avatarSpacePublicId;
        }

        public String getAvatarVariantType() {
            return avatarVariantType;
        }

        public String getAvatarVariantProfile() {
        ...
        public void setAuthorPublicId(byte[] v) {
            authorPublicId = v;
        }
        ...
        public void setAvatarAssetPublicId(byte[] v) {
            avatarAssetPublicId = v;
        }

        public void setAvatarSpacePublicId(byte[] v) {
            avatarSpacePublicId = v;
        }

        public void setAvatarVariantType(String v) {
            avatarVariantType = v;
        }

        public void setAvatarVariantProfile(String v) {
            avatarVariantProfile = v;
        }
    }
