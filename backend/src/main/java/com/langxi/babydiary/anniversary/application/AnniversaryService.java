...
public class AnniversaryService {
    ...
    public Item create(UUID spaceId, long accountId, Command command, boolean elevated) {
        ...
        anniversaries.insert(
                new AnniversaryRepository.NewAnniversary(
                        publicId,
                        space.internalId(),
                        accountId,
                        value.title(),
    ...
    @Transactional
    public Item update(
            UUID spaceId, UUID anniversaryId, long accountId, Command command, boolean elevated) {
        SpaceAccess.SpaceContext space = spaces.requireWriter(spaceId, accountId);
        require(space.internalId(), anniversaryId);
        Validated value = validate(spaceId, space.internalId(), accountId, command, elevated);
        if (!anniversaries.update(
                space.internalId(),
                anniversaryId,
                new AnniversaryRepository.UpdatedAnniversary(
                        value.title(),
                        value.date(),
                        value.description(),
                        value.coverAssetId(),
    ...
    private Validated validate(
            UUID spacePublicId, long spaceId, long accountId, Command command, boolean elevated) {
        String title = command.title() == null ? "" : command.title().trim();
        if (title.isBlank())
            throw ApiException.badRequest("ANNIVERSARY_TITLE_REQUIRED", "纪念日名称不能为空");
        if (title.length() > 100)
            throw ApiException.badRequest("ANNIVERSARY_TITLE_TOO_LONG", "纪念日名称不能超过100个字符");
        if (command.date() == null)
            throw ApiException.badRequest("ANNIVERSARY_DATE_REQUIRED", "请选择纪念日日期");
        String description =
        ...
        if (command.coverAssetId() != null) {
            ...
            coverId =
                    media
                            .findByPublicIds(spaceId, List.of(command.coverAssetId()), accountId)
                            .stream()
                            ...
                            .orElseThrow(
                                    () ->
                                            ApiException.badRequest(
                                                    "COVER_MEDIA_INVALID", "封面图片不存在或不属于当前空间"));
        }
        return new Validated(
                title, command.date(), description, coverId, Math.max(0, command.sortOrder()));
    }

    private Anniversary require(long spaceId, UUID publicId) {
        return anniversaries
                .findByPublicId(spaceId, publicId)
    ...
    private Item item(long spaceId, Anniversary value) {
        MediaAsset cover =
                value.coverAssetId() == null
                        ? null
                        : media
                                .findByPublicIdsInSpace(spaceId, List.of(value.coverAssetId()))
                                .stream()
                                .findFirst()
                                .orElse(null);
    ...
    public record Command(
            String title, LocalDate date, String description, UUID coverAssetId, int sortOrder) {}
