  ...
  <div class="shared-page">
    <main class="shared-container">
      ...
      <article v-else class="shared-article">
        <time>{{ formatChineseDate(diary.diaryDate) }}</time><h1>{{ diary.title }}</h1>
        <div class="shared-content" v-html="diary.contentHtml" />
...
import 'element-plus/es/components/button/style/css.mjs'; import 'element-plus/es/components/icon/style/css.mjs'; import 'element-plus/es/components/input/style/css.mjs'
const route = useRoute(); const password = ref(''); const diary = ref(null); const loading = ref(false)
const openShare = async () => { loading.value = true; try { diary.value = await workspaceApi.shares.open(route.params.token, password.value || null) } finally { loading.value = false } }
</script>
